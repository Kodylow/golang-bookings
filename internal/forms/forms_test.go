...
func TestForm_MinLength(t *testing.T) {
	...
	form.MinLength("x", 10)
	if form.Valid() {
		t.Error("form shows min length for nonexistent field")
	}

	isError := form.Errors.Get("x")
	if isError == "" {
		t.Error("should have an error but didn't")
	}
...
func TestForm_IsEmail(t *testing.T) {
	...
	postedValues = url.Values{}
	postedValues.Add("email", "kody@kody.com")
	form = New(postedValues)
	...
	postedValues = url.Values{}
	postedValues.Add("email", "kody.com")
	form = New(postedValues)

	form.IsEmail("email")
	if form.Valid() {
		t.Error("got valid email when we shouldn't have")
	}
}