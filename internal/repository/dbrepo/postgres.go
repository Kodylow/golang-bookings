...
//GetReservationByID returns one res by ID
...
//UpdateProcessedForReservation updates the processed status of a reservation in the database
func (m *postgresDBRepo) UpdateProcessedForReservation(id, processed int) error {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()

	query := `update reservations set id = $1 where id = $2`
...
func (m *postgresDBRepo) InsertBlockForRoom(id int, startDate time.Time) error {
	...
	query := `insert into room_restrictions (
						start_date, end_date, room_id, restriction_id
