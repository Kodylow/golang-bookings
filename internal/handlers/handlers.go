...
import (
	...
	"github.com/kodylow/golang-bookings/internal/repository/dbrepo"
)
...
func (m *Repository) PostReservation(w http.ResponseWriter, r *http.Request) {
	...
	htmlMessage := fmt.Sprintf(`
		...
		Hi %s! <br>
		This email is confirming your reservation from %s to %s.<br>
		We look froward to your stay!
	...
	htmlMessage = fmt.Sprintf("A reservation has been made for %s from %s to %s.", reservation.FirstName, reservation.StartDate.Format("2006-01-02"), reservation.EndDate.Format("2006-01-02"))
...
//ShowLogin routes to the login page
...
//PostShowLogin handles loggging the user in
func (m *Repository) PostShowLogin(w http.ResponseWriter, r *http.Request) {
	//whenever you login or logout you always renew the token
...
//Logout handles logging the user out by destroying the session
...
//AdminShowReservation shows a reservation by id
...
//AdminPostShowReservation shows a reservation by id
...
//AdminPostReservationsCalendar handles posting admin updates to the reservations calendar.
	...
	for _, x := range rooms {
		...
		for name, value := range curMap {
			if val, ok := curMap[name]; ok {
				//if entry in map not in posted data & restriction > 0, remove
				if val > 0 {
